...
import {setSelectedDataType, setSelectedIds} from '../../store/selection.reducer';
...
export const DataItemSelect: FC<Props> = ({items}) => {
    ...
    const selectItems = items
        .map((item: DataType) => getPresentDataType(item))
        .map((item: PresentDataType) => <MenuItem key={item.id + item.name} value={item.id}>{item.name}</MenuItem>);
